...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        hist = np.histogram(formants, bins=20, range=(0, 5500), density=False)
        return hist[0]
    ...
    def _compute_pitch_features(self, window):
        ...
        hist = np.histogram(pitch_contour, bins=16, density=False)
        return hist[0]
    ...
    def _compute_energy_features(self, window):
    ...
    def _compute_delta_coefficients(self, window, n=10):
        """
        ...
        denominator = 0
        for i in range(n+1):
            denominator += np.power(i, 2)
        denominator = denominator*2

        x = np.array([])
        numerator = np.zeros(n_coeffs, dtype=float)
        for i in range(n, n_frames-n):
            for j in range(1, n+1):
                numerator = np.add(numerator, np.multiply(np.subtract(mfcc_feats[i+j, :], mfcc_feats[i-j, :]), j))
            x = np.append(x, np.divide(numerator, denominator, dtype=float))
        x = x.flatten()
        return x

    def _compute_delta_delta_coefficients(self, window, n=10):
        x = self._compute_delta_coefficients(window, n)
        a = len(x) / 13
        x = np.reshape(x, (-1, 13))
        ...
        denominator = denominator * 2
        y = np.array([])
        numerator = np.zeros(13, dtype=float)
        for i in range(n, a-n):
            for j in range(1, n + 1):
                numerator = np.add(numerator, np.multiply(np.subtract(x[i + j, :], x[i - j, :]), j))
            y = np.append(y, np.divide(numerator, denominator, dtype=float))
        y = x.flatten()
    ...
    def extract_features(self, window, debug=True):
        ...
        x = np.append(x, self._compute_energy_features(window))
        return x    
